...
check_uninstall_file() {
    ...
    if [[ ! -f "$UNINSTALL_FILE_PATH" ]]; then
        log_error "$(_ "common.error_uninstall_file_not_found" "$UNINSTALL_FILE_PATH")"
        return 1
    fi
}
...
do_uninstall() {
    ...
    while IFS= read -r path || break; do
        ...
        if [[ -e "$path" || -L "$path" ]]; then
            log_info "$(_ "common.info_uninstall_delete" "$path")"
            rm -rf "$path" || {
                log_error "$(_ "common.error_uninstall_delete_failed" "$path")"
                return 1
            }
        else
            log_info "$(_ "common.info_uninstall_path_not_exists" "$path")"
        fi
...
#               $? - ошибка удаления
run_uninstall() {
    io::confirm_action "$(_ "common.info_uninstall_confirm")"
    check_uninstall_file

    log_info "$(_ "common.info_uninstall_start")"
    do_uninstall
    log_success "$(_ "common.info_uninstall_success")"
}