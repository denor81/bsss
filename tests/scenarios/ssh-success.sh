...
# @expected: Exit code 0, process lifecycle validated, BSSS SSH and UFW rules created
# @timeout: 30
...
# @type:        Orchestrator
# @description: Set up initial state for SSH success test
# @stdin:       none
# @stdout:      none
# @exit_code:   0 - success
#               1 - error
test::setup() {
    test::log_info "Setting up SSH success test..."
...
# @type:        Orchestrator
# @description: Clean up system state after SSH success test
# @stdin:       none
# @stdout:      none
# @exit_code:   0 - success
#               1 - error
test::cleanup() {
    test::log_info "Cleaning up SSH success test..."
    
    # Remove BSSS SSH rules created during test
    test::log_info "Cleaning SSH state..."
    ssh::delete_all_bsss_rules
    
    # Remove BSSS UFW rules created during test
    test::log_info "Cleaning UFW state..."
    ufw::delete_all_bsss_rules
...
# @type:        Orchestrator
# @description: Validate SSH success test results
# @stdin:       none
# @stdout:      none
...
test::custom_validate() {
    ...
    test::log_info "Validating SSH success test results..."
    
    # Check if BSSS SSH rule was created
    test::log_info "Checking BSSS SSH rule..."
    local ssh_config_path="${SSH_CONFIGD_DIR%/}/${BSSS_SSH_CONFIG_FILE_NAME}"
    if [[ ! -f "$ssh_config_path" ]]; then
        test::log_fail "Custom validation" "BSSS SSH config file not created: $ssh_config_path"
        return 1
    fi
    test::log_validation "BSSS SSH rule created: $ssh_config_path"
    
    # Check if UFW rule was created
    test::log_info "Checking UFW rule..."
    ...
    test::log_validation "BSSS UFW rule created ($ufw_rule_count rule(s))"
...
# @type:        Orchestrator
# @description: Run SSH success test scenario
# @stdin:       none
# @stdout:      none
...
test::run() {
    ...
    test::log_info "Running SSH success test"
