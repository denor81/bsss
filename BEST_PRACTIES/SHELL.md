...
2.  **Режимы безопасности (CRITICAL):** **ВЫ ДОЛЖНЫ** установить следующие три опции в начале скрипта для обеспечения безопасности и предсказуемости:
    *   `set -o errexit` (или `set -e`): Немедленно выйти, если какая-либо команда завершается неудачей.
    *   `set -o nounset` (или `set -u`): Выйти при попытке использования неустановленной переменной. Используйте `${VARNAME-}` для безопасного доступа к переменным, которые могут быть не установлены.
    *   `set -o pipefail`: Гарантировать, что конвейер (`|`) завершается неудачей, если какой-либо из его компонентов завершается неудачей.
    ```bash
    set -o errexit
    set -o nounset
    set -o pipefail
