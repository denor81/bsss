...
7. Карта функций и переиспользование кода
 ...
 8. Типы функций и их назначение
  ...
  - Validator (Валидатор): Проверяет условия и возвращает код возврата (0=истина, 1/ложь=ложь). Не принимает stdin, не выводит в stdout.
  - Orchestrator (Диспетчер): Принимает решения, управляет логикой запуска других функций.
  ...
  13. Стандарт нейминга функций
   ...
   14. Меню (Menu System)
      ...
      - Шаблон кода: [docs/menu/menu_template.sh](docs/menu/menu_template.sh)
       ...
       - **ИСКЛЮЧЕНИЯ** (не соответствуют шаблону):
         ...
         * `lib/i18n/language_installer.sh` — динамическое меню выбора языка (фиксированный список языков)
      ...
      - Чеклист для создания нового меню:
        ...
        3. Получить выбор через `io::ask_value` с правильным паттерном
         ...
         6. Добавить переводы в `lib/i18n/ru/common.sh`
   ...
   15. I18n (Internationalization)
      ...
      - Архитектура:
        ...
        * Файловая структура: lib/i18n/{ru|en}/common.sh (все переводы в одном файле для каждого языка)
     ...
     - Конвенции именования ключей:
      ...
      - Универсальные переменные переводов:
       ...
       - Файлы переводов:
         * lib/i18n/ru/common.sh - все переводы на русском (common, SSH, UFW, system, permissions, user modules)
         * lib/i18n/en/common.sh - все переводы на английском (common, SSH, UFW, system, permissions, user modules)

         - Добавление новых переводов:
           * **Правило для агентов-программистов**: При необходимости добавить новый лог (log_info, log_error и т.д.) нужно в первую очередь создавать переменную перевода (локализации) в файле русского языка (lib/i18n/ru/common.sh)
           ...
           * Процесс:
             1. Добавьте ключ в lib/i18n/ru/common.sh (обязательно)
...
lib/i18n/
 ...
 ├── ru/                         # Русские переводы
 │   └── common.sh               # Все переводы (common, SSH, UFW, system, permissions, user)
 └── en/                         # Английские переводы
     └── common.sh               # All translations (common, SSH, UFW, system, permissions, user)
```
